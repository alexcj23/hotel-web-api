...
public class CuartoDAOImpl implements CuartoDAO 
{
    ...
    {
        String sql = queryOption.build();
        return this.jdbcTemplate.query(sql, new BeanPropertyRowMapper<Cuarto>(Cuarto.class));
    }

    @Override
    public List<Cuarto> obtenerTodosPorCategoriaId(int pCategoriaId, QueryOption queryOption) 
    {
    	queryOption.setFilter(" where categoria = ?");
        final String sql = queryOption.build();
        return this.jdbcTemplate.query(sql, new Object[]{pCategoriaId}, new BeanPropertyRowMapper<Cuarto>(Cuarto.class));
    }
