...
public class InventarioServiceImpl implements InventarioService 
{
    ...
    @Override
    public Pagina<Cuarto> obtenerTodosCuarto(QueryOption queryOption) 
    {
        List<Cuarto> cuartos;
        final int count = this.cuartoDAO.contar();
        if(count > 0)
        {
            cuartos = this.cuartoDAO.obtenerTodos(queryOption);
        }
        else
        {
            cuartos = new ArrayList<Cuarto>();
        }
        return new Pagina<Cuarto>(cuartos, count,  queryOption.getOffset(), queryOption.getLimit());
    }

    @Override
    public Pagina<Cuarto> obtenerTodosCuartoEnCategoria(final int pCategoriaCuartoId, QueryOption queryOption)
    {
        final int count = this.cuartoDAO.contarPorCategoria(pCategoriaCuartoId);
        List<Cuarto> cuartos = null;
        if(count > 0)
        {
            cuartos = this.cuartoDAO.obtenerTodosPorCategoriaId(pCategoriaCuartoId, queryOption);
        }
        else
        {
            cuartos = new ArrayList<Cuarto>();
