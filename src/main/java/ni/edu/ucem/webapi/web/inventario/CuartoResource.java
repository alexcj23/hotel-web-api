...
public class CuartoResource {
	...
	//http://localhost:8080/v1/inventario/cuartos?paging=[offset:1,limit:20]&order=[numero:desc]
	...
	{
		final QueryOption qo = new QueryOption(request, Cuarto.class);

		Pagina<Cuarto> pagina;
		if (categoria != null) {
			pagina = this.inventarioService.obtenerTodosCuartoEnCategoria(categoria, qo);
		} else {
			pagina = this.inventarioService.obtenerTodosCuarto(qo);
		}
	...
	/**
	 * Para que la validación funcione solo es necesario agregar la
	 * anotación @valid al método. Spring se asegura que la petición entrante
	 * sea validado con las reglas definidas en el POJO.
	 * 
	 * @see <a href=
	 *      "http://docs.spring.io/spring-security/site/docs/current/reference/html/el-access.html#method-security-expressions">
	...
	/**
	 * Negociación de contenido. Aceptamos form-parameters para la creación de
	 * un nuevo recurso.
	 * 
	 * @param numero
	 * @param descripcion
	...
	public ApiResponse guardarCuarto(@PathVariable("id") final int id, @RequestBody final Cuarto cuartoActualizado) {
		final Cuarto cuarto = new Cuarto(id, cuartoActualizado.getNumero(), cuartoActualizado.getDescripcion(),
